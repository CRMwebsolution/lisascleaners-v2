import { createClient, type User } from "@supabase/supabase-js";
import { NextResponse } from "next/server";
import { DEFAULT_LISA_BUSINESS_ID } from "@/lib/site";

export const runtime = "nodejs";

type ProfileRow = {
  id: string;
  business_id?: string;
  full_name: string;
  role: "admin" | "staff";
  email?: string;
};

function adminClient() {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const anonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  return { supabaseUrl, anonKey, serviceKey };
}

async function requireAdmin(request: Request, bodyToken?: string) {
  const { supabaseUrl, anonKey, serviceKey } = adminClient();
  if (!supabaseUrl || !anonKey) return { error: NextResponse.json({ error: "Missing Supabase env" }, { status: 500 }) };
  const token = request.headers.get("authorization")?.replace(/^Bearer\s+/i, "") || bodyToken;
  if (!token) return { error: NextResponse.json({ error: "Sign in as an admin and try again." }, { status: 401 }) };
  if (!serviceKey) return { error: NextResponse.json({ error: "Add SUPABASE_SERVICE_ROLE_KEY to Vercel env, then redeploy." }, { status: 501 }) };
  const authed = createClient(supabaseUrl, anonKey, { global: { headers: { Authorization: `Bearer ${token}` } } });
  const { data: userData, error: userError } = await authed.auth.getUser(token);
  if (userError || !userData.user) return { error: NextResponse.json({ error: "Session expired. Sign in again." }, { status: 401 }) };
  const admin = createClient(supabaseUrl, serviceKey);
  const { data: actor } = await admin.from("lisa_profiles").select("id, role, full_name").eq("id", userData.user.id).maybeSingle();
  if (actor?.role !== "admin") return { error: NextResponse.json({ error: "Admin only." }, { status: 403 }) };
  return { admin, actorId: userData.user.id, actor };
}

async function findAuthUserByEmail(admin: ReturnType<typeof createClient>, email: string) {
  const target = email.trim().toLowerCase();
  for (let page = 1; page <= 20; page += 1) {
    const { data, error } = await admin.auth.admin.listUsers({ page, perPage: 200 });
    if (error) throw new Error(error.message);
    const match = data.users.find((user) => (user.email ?? "").toLowerCase() === target);
    if (match) return match;
    if (data.users.length < 200) return null;
  }
  return null;
}

async function saveProfile(admin: ReturnType<typeof createClient>, row: {
  id: string;
  business_id: string;
  full_name: string;
  role: "admin" | "staff";
  email: string;
}) {
  const payloads: ProfileRow[] = [
    row,
    { id: row.id, business_id: row.business_id, full_name: row.full_name, role: row.role, email: row.email },
    { id: row.id, business_id: row.business_id, full_name: row.full_name, role: row.role },
    { id: row.id, full_name: row.full_name, role: row.role },
  ];
  let lastError = "Could not save staff profile.";
  for (const payload of payloads) {
    const insert = await admin.from("lisa_profiles").upsert(payload as never, { onConflict: "id" });
    if (!insert.error) return null;
    lastError = insert.error.message;
    if (/already exists|duplicate|unique/i.test(insert.error.message) && row.email) {
      const existing = await admin.from("lisa_profiles").select("id").eq("email", row.email).maybeSingle();
      if (existing.data?.id) {
        const update = await admin.from("lisa_profiles").update({
          full_name: row.full_name,
          role: row.role,
          business_id: row.business_id,
        } as never).eq("id", existing.data.id);
        if (!update.error) return null;
        lastError = update.error.message;
      }
    }
  }
  return lastError;
}

export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as {
    email?: string;
    password?: string;
    full_name?: string;
    role?: "admin" | "staff";
    access_token?: string;
  } | null;
  if (!body?.email || !body.full_name || !body.role) {
    return NextResponse.json({ error: "Name, email, and role are required." }, { status: 400 });
  }
  if (!["admin", "staff"].includes(body.role)) {
    return NextResponse.json({ error: "Role must be admin or staff." }, { status: 400 });
  }
  const gate = await requireAdmin(request, body.access_token);
  if ("error" in gate && gate.error) return gate.error;
  const admin = gate.admin!;

  const email = body.email.trim();
  let user: User | null = await findAuthUserByEmail(admin, email);
  let reused = Boolean(user);
  if (!user) {
    if (!body.password || body.password.length < 6) {
      return NextResponse.json({ error: "A password of at least 6 characters is required for a new login." }, { status: 400 });
    }
    const created = await admin.auth.admin.createUser({
      email,
      password: body.password,
      email_confirm: true,
      user_metadata: { full_name: body.full_name.trim(), role: body.role },
    });
    if (created.data.user) user = created.data.user;
    else if (/already|registered|exists|duplicate/i.test(created.error?.message ?? "")) {
      user = await findAuthUserByEmail(admin, email);
      reused = true;
    } else {
      return NextResponse.json({ error: created.error?.message ?? "Could not create login." }, { status: 400 });
    }
  }
  if (!user) return NextResponse.json({ error: "Could not find or create that login." }, { status: 400 });

  const updates: { email_confirm: true; user_metadata: Record<string, string>; password?: string } = {
    email_confirm: true,
    user_metadata: { full_name: body.full_name.trim(), role: body.role },
  };
  if (!reused && body.password) updates.password = body.password;
  await admin.auth.admin.updateUserById(user.id, updates);

  const profileError = await saveProfile(admin, {
    id: user.id,
    business_id: process.env.LISA_BUSINESS_ID || DEFAULT_LISA_BUSINESS_ID,
    full_name: body.full_name.trim(),
    role: body.role,
    email,
  });
  if (profileError) return NextResponse.json({ error: profileError }, { status: 400 });
  return NextResponse.json({ ok: true, id: user.id, reused });
}

export async function DELETE(request: Request) {
  const body = (await request.json().catch(() => null)) as {
    id?: string;
    reassign_to?: string;
    access_token?: string;
  } | null;
  if (!body?.id) return NextResponse.json({ error: "Staff id is required." }, { status: 400 });
  const gate = await requireAdmin(request, body.access_token);
  if ("error" in gate && gate.error) return gate.error;
  const admin = gate.admin!;
  const actorId = gate.actorId!;
  if (body.id === actorId) return NextResponse.json({ error: "You cannot delete your own login." }, { status: 400 });

  const today = new Date().toISOString().slice(0, 10);
  const { data: assignments } = await admin.from("lisa_job_assignments").select("id, job_id").eq("assignee_id", body.id);
  const jobIds = [...new Set((assignments ?? []).map((row: { job_id: string }) => row.job_id))];
  const { data: futureJobs } = jobIds.length
    ? await admin.from("lisa_jobs").select("id, customer_name, job_date, job_time, status").in("id", jobIds).gte("job_date", today).neq("status", "cancelled")
    : { data: [] as { id: string; customer_name: string; job_date: string; job_time: string | null; status: string }[] };
  const future = futureJobs ?? [];
  const reassignTo = body.reassign_to || actorId;
  if (future.length && !reassignTo) {
    return NextResponse.json({ error: "Reassign future jobs before deleting this person.", jobs: future }, { status: 409 });
  }
  if (reassignTo === body.id) {
    return NextResponse.json({ error: "Pick someone else to take the future jobs." }, { status: 400 });
  }

  const businessId = process.env.LISA_BUSINESS_ID || DEFAULT_LISA_BUSINESS_ID;
  for (const job of future) {
    const { data: existing } = await admin.from("lisa_job_assignments").select("id").eq("job_id", job.id).eq("assignee_id", reassignTo).maybeSingle();
    const current = (assignments ?? []).find((row: { job_id: string }) => row.job_id === job.id);
    if (existing?.id) {
      if (current) await admin.from("lisa_job_assignments").delete().eq("id", current.id);
    } else if (current) {
      const moved = await admin.from("lisa_job_assignments").update({ assignee_id: reassignTo } as never).eq("id", current.id);
      if (moved.error) {
        await admin.from("lisa_job_assignments").insert({ job_id: job.id, assignee_id: reassignTo, business_id: businessId } as never);
        await admin.from("lisa_job_assignments").delete().eq("id", current.id);
      }
    } else {
      await admin.from("lisa_job_assignments").insert({ job_id: job.id, assignee_id: reassignTo, business_id: businessId } as never);
    }
  }

  await admin.from("lisa_job_assignments").delete().eq("assignee_id", body.id);
  const removed = await admin.from("lisa_profiles").delete().eq("id", body.id);
  if (removed.error) return NextResponse.json({ error: removed.error.message }, { status: 400 });
  await admin.auth.admin.deleteUser(body.id);
  return NextResponse.json({ ok: true, reassigned: future.length });
}
