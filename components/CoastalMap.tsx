  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-purple-light bg-white">
      <svg viewBox="0 0 100 86" className="h-auto w-full" role="img" aria-label="Map of Crystal Coast towns Lisa serves">
        <rect width="100" height="86" fill="#eaf4f2" />
        <path d="M0 58 C18 52, 30 68, 48 64 C62 61, 74 54, 100 58 L100 86 L0 86 Z" fill="#b9d9d4" />
        <path d="M0 70 C22 64, 40 76, 58 72 C74 69, 86 64, 100 68 L100 86 L0 86 Z" fill="#7eb8b3" />
        <path d="M8 40 C22 28, 38 24, 54 30 C66 22, 80 26, 94 34" fill="none" stroke="#16324f" strokeWidth="0.6" strokeDasharray="1.2 1.2" />
        {pins.map((pin) => (
          <g key={pin.name}>
            <circle cx={pin.x} cy={pin.y} r={pin.name === "Newport" ? 2.2 : 1.6} fill={pin.name === "Newport" ? "#16324f" : "#2a7f74"} />
            <text x={pin.x} y={pin.y - 3} textAnchor="middle" fontSize="3.1" fill="#16324f" fontFamily="Georgia, serif">{pin.name}</text>
          </g>
        ))}
      </svg>
