...
async function loadJobsWithAssignments() {
  ...
  const [{ data: jobRows, error: jobError }, { data: assignmentRows, error: assignError }, { data: people }] = await Promise.all([
    supabase.from("lisa_jobs").select("*").order("job_date"),
  ...
  return {
    jobs,
    profiles,
    error: jobError?.message || assignError?.message || null,
  };
}
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day.toISOString()} className="min-h-24 rounded-md bg-white p-2 text-xs">
              ...
              {dayJobs.map((job) => <p key={job.id} className="mt-1 truncate">{jobTimeLabel(job.job_time)} {job.customer_name}</p>)}
            </div>
          );
        })}
      </div>
