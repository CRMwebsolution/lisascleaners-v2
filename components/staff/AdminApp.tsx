...
import type { JobWithAssignments, LisaProfile, QuoteRequest, StaffRole } from "@/lib/types";
...
export default function AdminApp() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: reqs }, { data: jobRows }, { data: people }] = await Promise.all([
      supabase.from("lisa_quote_requests").select("*").order("created_at", { ascending: false }),
      supabase.from("lisa_jobs").select("*, job_assignments:lisa_job_assignments(*, profile:lisa_profiles(*))").order("job_date"),
      supabase.from("lisa_profiles").select("*").order("full_name"),
    ]);
    setRequests((reqs as QuoteRequest[]) ?? []);
    setJobs((jobRows as JobWithAssignments[]) ?? []);
    setProfiles((people as LisaProfile[]) ?? []);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-purple-soft lg:flex">
      ...
      <main className="flex-1 p-4 lg:p-6">
        ...
        {section === "requests" ? (
          <Requests requests={requests} onStatus={async (id, status) => {
        ...
        {section === "calendar" ? <Calendar jobs={jobs} calDate={calDate} calView={calView} onView={setCalView} onDate={setCalDate} /> : null}
        {section === "jobs" ? <Jobs jobs={jobs} profiles={profiles} draft={draft} sourceRequest={sourceRequest} currentUserId={profile?.id ?? ""} onError={setError} onClear={() => { setDraft(null); setSourceRequest(null); }} onSaved={async () => { setDraft(null); setSourceRequest(null); await load(); }} /> : null}
        {section === "staff" ? <Staff profiles={profiles} onSaved={load} onError={setError} /> : null}
        {section === "documents" ? <Docs jobs={jobs} requests={requests} /> : null}
      </main>
    </div>
  );
}

function Requests({ requests, onCreate, onStatus }: { requests: QuoteRequest[]; onCreate: (req: QuoteRequest) => void; onStatus: (id: string, status: QuoteRequest["status"]) => void }) {
  return (
    <section>
      ...
      <ul className="mt-4 space-y-3">
        ...
        {requests.map((req) => (
          <li key={req.id} className="rounded-md border border-purple-light bg-white p-4">
            <div className="flex justify-between gap-2"><p className="font-semibold text-purple-dark">{req.name}</p><span className="text-xs uppercase text-purple-mid">{req.status}</span></div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {(["new", "contacted", "booked", "declined"] as const).map((status) => (
                <button key={status} type="button" className="tap rounded-md border border-purple-light px-3 text-sm capitalize" onClick={() => onStatus(req.id, status)}>{status}</button>
              ))}
            </div>
          </li>
        ))}
...
function Calendar({ jobs, calDate, calView, onView, onDate }: { jobs: JobWithAssignments[]; calDate: Date; calView: CalView; onView: (view: CalView) => void; onDate: (date: Date) => void }) {
  const title = calView === "month" ? format(calDate, "MMMM yyyy") : calView === "week" ? `Week of ${format(startOfWeek(calDate), "MMM d, yyyy")}` : format(calDate, "EEEE, MMMM d, yyyy");
  ...
  return (
    <section>
      ...
      <div className={`mt-4 grid gap-2 ${calView === "day" ? "grid-cols-1" : "grid-cols-7"}`}>
        {days.map((day) => {
          const dayJobs = jobs.filter((job) => isSameDay(parseISO(job.job_date), day));
          return (
            <div key={day.toISOString()} className="min-h-24 rounded-md bg-white p-2 text-xs">
              <p className="font-semibold text-purple-dark">{format(day, calView === "month" ? "d" : "EEE d")}</p>
              {dayJobs.map((job) => <p key={job.id} className="mt-1 truncate">{job.job_time.slice(0, 5)} {job.customer_name}</p>)}
            </div>
          );
        })}
      </div>
...
function Jobs({ jobs, profiles, draft, sourceRequest, currentUserId, onClear, onSaved, onError }: { jobs: JobWithAssignments[]; profiles: LisaProfile[]; draft: Partial<JobWithAssignments> | null; sourceRequest: QuoteRequest | null; currentUserId: string; onClear: () => void; onSaved: () => Promise<void>; onError: (message: string | null) => void }) {
    ...
    <section className="grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
      ...
      <div>
        ...
        <ul className="mt-3 space-y-2">
          {jobs.map((job) => (
            <li key={job.id} className="rounded-md bg-white p-3 text-sm">
              <p className="font-semibold">{job.customer_name} · {job.status}</p>
              <p>{job.job_date} {job.job_time.slice(0, 5)} · {job.type_of_clean}</p>
              <p>{job.address}</p>
...
function Docs({ jobs, requests }: { jobs: JobWithAssignments[]; requests: QuoteRequest[] }) {
  ...
  return (
    <section className="max-w-xl rounded-md bg-white p-4">
      ...
      <div className="mt-4 space-y-3">
        <select className={inputCls} value={kind} onChange={(e) => setKind(e.target.value as "quote" | "invoice")}><option value="quote">Quote</option><option value="invoice">Invoice</option></select>
        <select className={inputCls} onChange={(e) => {
          ...
          <option value="">Prefill from job or request</option>
        ...
        <input className={inputCls} placeholder="Number" value={number} onChange={(e) => setNumber(e.target.value)} />
        <input className={inputCls} placeholder="Customer" value={customer} onChange={(e) => setCustomer(e.target.value)} />
        <input className={inputCls} placeholder="Address" value={address} onChange={(e) => setAddress(e.target.value)} />
        <input className={inputCls} placeholder="Line item" value={item} onChange={(e) => setItem(e.target.value)} />
        ...
        <button type="button" className="tap rounded-md bg-purple-mid px-4 text-sm font-semibold text-white" onClick={() => downloadLisaPdf({ kind, number, customer, address, date: format(new Date(), "yyyy-MM-dd"), lines: [{ item, qty: Number(qty) || 1, price: Number(price) || 0 }] })}>
